...
class SearchSurahAyah extends StatefulWidget {
  ...
  @override
  _SearchSurahAyahState createState() => _SearchSurahAyahState();
}

class _SearchSurahAyahState extends State<SearchSurahAyah> {
  String _myActivity;
  ...
  _saveForm() async {
    ...
    if (form.validate()) {
      ...
      setState(() {
        _myActivityResult =
            'Surat ${dataSourceSurah[mySurah - 1]['nama_latin']} : ${myAyah}';
      });
      ayatindex = int.parse(myAyah);
      assert(ayatindex is int);

      suratindex = int.parse(mySurah.toString());
      assert(suratindex is int);
      print('=========$suratindex');

      await getAyat(suratindex);
      // setState(() {
      //   myAyah = null;
      //   data = null;
      // });
  ...
  String surahValidator(var surah) {
    surah = mySurah;
    if (surah == '') {
      return 'Surah kosong';
    }
    // else if (surah.length < 3) {
    //   return 'panjang string surah';
    // }
    return null;
  }

  String ayahValidator(var ayah) {
    ayah = myAyah;
    if (ayah == '') {
      return 'Ayah kosong';
    }
    // else if (ayah.length < 3) {
    //   return 'panjang string ayah';
    // }
    return null;
  }
  ...
  Future<AyatList> getAyat(int index) async {
    // int indexAyat = index > 1 ? index - 1 : index;
    String url = "https://quranapi.idn.sch.id/surah/${index}";
    ...
    if (response.statusCode == 200) {
      ...
      print('ayat : $index');
      // print('url : https://equran.id/api/surat/$index');
                  ...
                  child: DropDownFormField(
                    ...
                    onChanged: (value) {
                      setState(() {
                        mySurah = value;
                        jumlah_ayat =
                            dataSourceSurah[mySurah - 1]['jumlah_ayat'];
                      });
                      dataSourceAyahsFunc();
                      print(
                          'jumlah ayat surah ${dataSourceSurah[mySurah - 1]['nama_latin']} : ${dataSourceSurah[mySurah - 1]['jumlah_ayat']}');
                    },
                    validator: surahValidator,
                    dataSource: dataSourceSurah,
                    textField: 'nama_latin',
                    valueField: 'nomor',
                  ),
                ),
                Container(
                  ...
                  child: DropDownFormField(
                    ...
                    validator: ayahValidator,
                    dataSource: dataSourceAyats,
                    textField: 'display',
                    valueField: 'value',
                  ),
                ),
