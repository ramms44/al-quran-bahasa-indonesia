...
class SurahAyats extends StatefulWidget {
  // audio
  ...
  State<SurahAyats> createState() => _SurahAyatsState(ayatIndex);
}
...
class _SurahAyatsState extends State<SurahAyats> {
  ...
  _SurahAyatsState(this.ayatIndex);

  var searchString = '';
  final _formKey = GlobalKey<FormState>();
  final _formKey_notes = GlobalKey<FormState>();
  ...
  //   );
  //   setState(() {
  //     alafasiDataSurat = alafasiData['surahs'][ayatIndex - 1]['ayahs'];
  //     first = alafasiDataSurat[0]['number'];
  //     // alafasiData['surahs'][0]['ayahs'][0]['audio']
  //   });
  //   print('alafasi data number pertama : ${alafasiDataSurat[0]['number']}');
  // }

  setNumberAyatPertama(index) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setInt('indexAyatPertama', index);
  }
  ...
  void _init() async {
    var length;
    setState(() {
      jumlahAyat = widget.ayatsList == null
          ? (widget.jumlah_ayat)
  ...
  var dataTafsir;
  Future<dynamic> getTafsir(int index) async {
    String url = "https://equran.id/api/tafsir/$index";
    final response = await http.get(Uri.parse(url));
  ...
  void setInitialPlaylist(length) async {
    int lengthAyah = length;
    if (ayatIndex > 2) {
      setState(() {
        lengthAyah = lengthAyah + widget.indexAudio;
      });
    } else {
      // code...
    }
  ...
  Future<AyatList> getAyatUthmani() async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        dataalfarisi = json.decode(response.body);
      });
  ...
  Widget build(BuildContext context) {
    ...
    if ((widget.ayatIndex != 1 && widget.ayatIndex != 9)) {
      setState(() {
        isVisible = false;
      });
    } else {
  ...
  Widget flexibleAppBar(BuildContext context, double width, double height) {
    return FlexibleSpaceBar(
      centerTitle: true,
      title: (widget.ayatIndex != 1 && widget.ayatIndex != 9)
          ? Container(
              padding: const EdgeInsets.only(top: 7, bottom: 7),
              width: double.infinity,
              child: Text(
                data == null ? '' : 'بِسْمِ اللّٰهِ الرَّحْمٰنِ الرَّحِيْمِ',
                textAlign: TextAlign.center,
                style: TextStyle(
